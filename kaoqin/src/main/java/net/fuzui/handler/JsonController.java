...
import net.fuzui.pojo.old_Attendance;
...
public class JsonController {
	...
	//获取json格式 从Android端传来的json格式，如果不是json格式，这里就接收不到数据
		...
		public @ResponseBody Course toTeacherAttendanceStart(@RequestBody old_Attendance  attendance ) throws Exception{
			...
			old_Attendance	attendance1=new old_Attendance();
			...
			long count=mongoTemplate.count(query1, old_Attendance.class)+1;
		...
		public @ResponseBody Course toTeacherAttendanceEnd(@RequestBody old_Attendance  attendance ) throws Exception{
			...
			long count=mongoTemplate.count(query1, old_Attendance.class);
			query1.addCriteria(Criteria.where("count").is(count));
			List<old_Attendance> attendancelist =mongoTemplate.find(query1,old_Attendance.class);
			...
			mongoTemplate.upsert(query1, update,old_Attendance.class);
			...
			List<old_Attendance> attendancelist1 =mongoTemplate.find(query1,old_Attendance.class);
		...
		public @ResponseBody User toStudentAttendanceStart(@RequestBody old_Attendance  attendance ) throws Exception{
			...
			if(mongoTemplate.count(query1, old_Attendance.class)<=0) {		
